...
TEST(Polinom, can_correctly_delete_monom_0)
{
    Polinom p(0);
    monom mn(1,1);
    monom mn2(2,1);
...
TEST(Polinom, can_correctly_delete_monom_2)
{
    ...
    p.addMonom(mn3);
    p.delMonom(2);
    ASSERT_ANY_THROW(p.get_monom(2));
}
...
TEST(Polinom, can_correctly_equate){
    ...
    Polinom p1(0);
    p1 = p;
    EXPECT_EQ(p1.get_monom(1).getCoef(), 2);
}
