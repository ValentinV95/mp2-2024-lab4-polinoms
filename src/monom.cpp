...
Monom::Monom(double k_, int degree_) {
    if (degree_ > 999 || degree_ < 0)
        throw out_of_range("Degree cant be more than 999 or less than 0");
    k = k_;
    degree = degree_;
    next = nullptr;
}
...
void Monom::check(const Monom& s)
{
    int x1 = degree / 100;
    int x2 = s.degree / 100;
    if (x1 + x2 > 9)
...
Monom  Monom::operator*(const Monom& n)
{
    check(n);
    Monom res;
    res.k = k * n.k;
